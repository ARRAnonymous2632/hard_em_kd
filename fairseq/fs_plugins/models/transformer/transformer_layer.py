...
from fs_plugins.modules.revpos_multihead_att import MultiHeadedAttentionRelative
...
class TransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        skip_self_attn_residual=False,
        skip_enc_attn_residual=False,
    ):
        """
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
        ...
        if not skip_self_attn_residual:
            x = self.residual_connection(x, residual)
...
class TransformerDecoderLayerRevPos(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, is_first=False
    ):
        super().__init__()
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn = self.build_encoder_attention(self.embed_dim, args, is_first)
    ...
    def build_encoder_attention(self, embed_dim, args, is_first=False):
        if is_first:
            return MultiHeadedAttentionRelative(
                1024,
                embed_dim=embed_dim,
                num_heads=args.decoder_attention_heads,
...
class TransformerSharedDecoderLayer(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False,
            encoder_att=None, self_att=None, fc1=None, fc2=None
    ):
        super().__init__()
        self.embed_dim = args.decoder_embed_dim
        self.dropout_module = FairseqDropout(
