...
def _main(cfg: DictConfig, output_file):
    ...
    target_losses = torch.zeros(len(task.dataset(cfg.dataset.gen_subset)))
    def decode_fn(x):
        if bpe is not None:
            x = bpe.decode(x)
    ...
    for sample in progress:
        ...
        loss_per_sample = criterion.get_per_sample_loss(models[0], sample)
