...
def load_checkpoint_to_cpu(path, arg_overrides=None, load_on_all_ranks=False):
    ...
    try:
        ...
        from scripts.average_checkpoints import last_n_checkpoints
        path_name = os.path.dirname(path)
        latest_path = last_n_checkpoints([path_name], 1, 'update')
        print("!!! I am recovering by loading the latest checkpoint by update num")
        with PathManager.open(latest_path[0], "rb") as f:
            state = torch.load(f, map_location=torch.device("cpu"))
