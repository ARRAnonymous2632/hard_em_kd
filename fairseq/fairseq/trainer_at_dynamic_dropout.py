...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                )  # NOTE: Change to NOT strict load, may cause futher problems. 
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        if getattr(self, "model_dual", None) is not None:
            extra_kwargs["model_dual"] = self.model_dual
        ...
        self.model.update_loss_distribution(logging_outputs[0]["loss_per_sample"])
    ...
    def _fast_stat_sync_sum(
        ...
        *logging_outputs* cannot contain any nested dicts/lists.
        ...
        if len(logging_outputs) > 0:
            ...
            for k in log_keys:
                if torch.is_tensor(logging_outputs[0][k]) and logging_outputs[0][k].dim() > 0:
                    v = [log[k] for log in logging_outputs if k in log]
                    data_concat = v
                else:
                    if not ignore:
        ...
        if data_concat:
            data_concat = distributed_utils.all_gather_concatenate(data_concat)
            data_concat = torch.concat(data_concat)

            data["logging_outputs_loss_per_sample"] = data_concat
