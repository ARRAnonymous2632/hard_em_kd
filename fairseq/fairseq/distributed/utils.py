...
def all_gather_concatenate(local_tensors):
    ...
    local_tensors_cpu = [tensor.cpu() for tensor in local_tensors]

    gathered_tensors = [None for _ in range(world_size)]
    dist.all_gather_object(gathered_tensors, local_tensors_cpu)

    # Concatenate the lists
    concatenated = []
    for tensors in gathered_tensors:
        concatenated.extend(tensors)

    # Move back to original device if necessary
    concatenated = [tensor.to(local_tensors[0].device) for tensor in concatenated]
